...
const PointTable = () => {
  ...
  async function fetchData() {
    ...
    try {
      ...
      setData(result.pointsTable[0].pointsTableInfo);
    } catch (error) {
      console.error(error);
    }
  }
  useEffect(() => {
    //fetchData();
    setData(fixedData.pointsTable[0].pointsTableInfo);
  }, []);
  return (
    <div>
      {data.length <= 0 ? (
        ...
        <table className="w-full table-auto border-collapse">
          <thead>
            <tr className="bg-white/20 h-[60px] border-gray-400 border-b-[1px]">
              <th className="w-[35%] md:w-[50%] text-left p-2">
                Teams
              </th>
              {/* <th className="ay-200">Images</th> */}
              <th className="text-left p-1 md:p-2">M</th>
              <th className="text-left p-1 md:p-2">W</th>
              <th className="text-left p-1 md:p-2">L</th>
              <th className="text-left p-1 md:p-2">NRR</th>
              <th className="text-left p-1 md:p-2 font-bold">Pts</th>
              <th className="p-1 md:p-2 text-center">Last 5</th>
            </tr>
          </thead>
          <tbody>
            {data.map((team, i) => (
              <tr key={i} className="border-gray-400/10 border-b-[1px] h-[35px] md:h-[50px]">
                <div className="flex gap-2 md:gap-6 mt-3">
                <td className="text-left p-1 md:p-2 w-2 ">{i + 1}</td>
                <td className="text-left p-1 md:p-2">
                  <img
                    className="w-4 object-contain"
                    src={`https://res.cloudinary.com/digkgdovw/image/upload/v1715267650/iplTeamLogo/${team.teamImageId}`}
                    alt=""
                  />
                  </td>

                <td className="text-left p-1 md:p-2">{team.teamName}</td>
                </div>
                <td className="text-left p-1 md:p-2">{team.matchesPlayed}</td>
                <td className="text-left p-1 md:p-2">{team.matchesWon}</td>
                <td className="text-left p-1 md:p-2">{team.matchesLost}</td>
                <td className="text-left p-1 md:p-2">{team.nrr}</td>
                <td className="text-left p-1 md:p-2 font-bold">{team.points}</td>
                <td className="justify-center p-1 md:p-2 flex gap-1 md:gap-2">
                  {team.form
                    .reverse()
                    .map((data) =>
                      data == "W" ? (
